...
predicate_test_numbers_are_positive = ! "$$FILE" | grep -E "\-[0123456789]"

predicate_test_numbers_are_in_order = "$$FILE" | grep -E "[0123456789]" | sort -n -c - 2>/dev/null

predicate_test_every_third_output_is_fizz = [ "$$("$$FILE" | wc -l)" -gt 2 ]  && [ "$$("$$FILE" | awk 'NR % 3 == 0' | grep -v "Fizz" | wc -l)" -eq 0 ]

predicate_test_every_fifth_output_is_buzz = [ "$$("$$FILE" | wc -l)" -gt 4 ]  && [ "$$("$$FILE" | awk 'NR % 5 == 0' | grep -v "Buzz" | wc -l)" -eq 0 ]

predicate_test_every_fifteenth_output_is_fizzbuzz = [ "$$("$$FILE" | wc -l)" -gt 14 ]  && [ "$$("$$FILE" | awk 'NR % 15 == 0' | grep -v "FizzBuzz" | wc -l)" -eq 0 ]
...
predicate_test_numbers_corespond_to_ordinal_position_counting_from_1 = "$$FILE" | awk '/^[0-9]*$$/ {if (NR != $$0) exit 1}'
